...
/**
 * Session manager which sets and retrieves the session from a request at a
 * custom header (Not a cookie)
 * 
 * @author gabriel_titerlea
 */
public class SessionManager extends DefaultSessionManager implements WebSessionManager {
  private static final Logger log = LoggerFactory
      .getLogger(DefaultWebSessionManager.class);

  /**
   * The name of the header used as a session id.
   */
  private static final String OXY_TOKEN_HEADER = "OXY-TOKEN";
  ...
  public void setSessionIdUrlRewritingEnabled(
  ...
  private void storeSessionId(Serializable currentId, HttpServletRequest request, HttpServletResponse response) {
    ...
    response.addHeader(OXY_TOKEN_HEADER, idString);
  }
  ...
  private String getSessionIdCookieValue(ServletRequest request, ServletResponse response) {
    ...
    return httpRequest.getHeader(OXY_TOKEN_HEADER);
  }
  ...
  private Serializable getReferencedSessionId(ServletRequest request,
    ...
    if (id != null) {
      request.setAttribute(
          ...
          ShiroHttpServletRequest.COOKIE_SESSION_ID_SOURCE); // TODO: change this to oxy header?
    } else {
      // Won't go in here
  ...
  private String getUriPathSegmentParamValue(ServletRequest servletRequest,
    ...
    uri = uri.substring(index + 1); // uri now contains only the path segment
                                    // params
  ...
  private String getSessionIdName() {
    String name = this.sessionIdCookie != null ? this.sessionIdCookie.getName()
        : null;
    if (name == null) {
  ...
  protected void onStart(Session session, SessionContext context) {
    ...
    request
        .removeAttribute(ShiroHttpServletRequest.REFERENCED_SESSION_ID_SOURCE);
  ...
  private void onInvalidation(SessionKey key) {
    ...
    if (request != null) {
      request
          .removeAttribute(ShiroHttpServletRequest.REFERENCED_SESSION_ID_IS_VALID);
    }
